...
val SIMPLE_COIN_EARNING = Earning((1.5*SLOT_PRICE).toInt(), 3*SLOT_PRICE)
val MULTIPLE_COIN_EARNING = Earning(3*SLOT_PRICE, 5*SLOT_PRICE)
val COIN_BAG_EARNING = Earning(5*SLOT_PRICE, 10*SLOT_PRICE)

val JOKER_EARNING = Earning(0, 100*SLOT_PRICE)

val HEALTH_EARNING = Earning(round(0.1*HERO_HEALTH).toInt(), round(0.25*HERO_HEALTH).toInt())
